...
col1, col2, col3 = st.columns([2, 2, 1.2])
with col1:
    st.markdown("**2. As initial**")
    st.latex(r'A_s = \frac{M_u}{\phi f_y jd \cdot d}')
with col2:
    st.code(f'{Mu*1e6:.1e}/({phi}×{fy}×{jd}×{d})', language='python')
with col3:
    st.metric("", f"{As_initial:.1f} mm²")
...
with col3:
    st.metric("", f"{As_calculated:.1f} mm²")

# Step 5: As min
col1, col2, col3 = st.columns([2, 2, 1.2])
with col1:
    st.markdown("**5. As,min**")
    st.latex(r'A_{s,min} = \frac{1.4 bd}{f_y}')
with col2:
    st.code(f'(1.4×{b}×{d})/{fy}', language='python')
with col3:
    st.metric("", f"{As_min:.1f} mm²")
...
with col2:
    st.code(f'({d}-{c:.2f})/{c:.2f}×0.003', language='python')
with col3:
    st.metric("", f"{es:.5f}")
...
with col1:
    ...
    for size, area in rebar_sizes[:3]:
        ...
        if num_bars <= 10:
            st.caption(f"• {num_bars}Ø{size} = {num_bars*area:.0f} mm²")
...
with col2:
    ...
    st.caption(f"• εs: {'✓' if strain_safe else '✗'} ({es:.5f})")
    st.caption(f"• Capacity: {'✓' if capacity_safe else '✗'} ({phi_Mn/Mu:.2f})")
    st.caption(f"• Type: {strain_status}")

st.markdown("---")
st.caption("ACI 318 Code")
